  ...
  helmet({
    ...
    contentSecurityPolicy: {
      ...
      directives: {
        ...
        'frame-src': ["'self'"],
        'img-src': ["'self'", 'data:', 'https://cdn.pixabay.com', 'https://cdnjs.cloudflare.com'],
        'script-src': [
          "'strict-dynamic'",
          "'self'",
          // @ts-expect-error
...
if (MODE === 'development') {
  ...
  chokidar.watch(watchPath, { ignoreInitial: true }).on('add', reloadBuild).on('change', reloadBuild)
}
